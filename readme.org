...
Enable flyspell in all text modes and in those places in source where you write comments. This uses either ispell or aspell on Linux (or MacOS, you can =brew install ispell=), but on Windows you have to install Hunspell and the appropriate dictionary. 
...
(cond ((eq system-type 'windows-nt)
       ...
       ;; Assuming this is where "brew install ispell" puts it.
